...
export default function Home() {
  ...
  return (
    <Authenticator>
      {({ signOut }) => (
        <div className="chat-app min-h-screen bg-[#f8fafc] text-slate-900">
          <div className="relative mx-auto flex min-h-screen max-w-6xl flex-col bg-white shadow-[0_20px_80px_rgba(15,23,42,0.08)]">
            <header className="flex items-center justify-between border-b border-slate-200 px-6 py-5">
              <div className="flex items-center gap-4">
                ...
                <span className="text-2xl font-bold tracking-[0.15em] uppercase text-slate-900">simonsays</span>
              </div>
              <button
                onClick={signOut}
                className="rounded-full border border-slate-300 bg-slate-50 px-4 py-2 text-sm font-medium text-slate-900 shadow-sm transition hover:bg-slate-100"
              >
                Sign Out
              </button>
            </header>
            ...
            <div className="chat-bar fixed bottom-0 left-0 right-0 z-40 flex items-center gap-3 border-t border-slate-200 bg-white/95 px-6 py-4 backdrop-blur-xl shadow-[0_-20px_40px_rgba(15,23,42,0.08)] sm:px-8">
              <input
                className="flex-1 rounded-full border border-slate-200 bg-slate-50 px-5 py-4 text-base text-slate-900 placeholder:text-slate-400 outline-none focus:border-slate-400 focus:ring-2 focus:ring-slate-200"
                value={input}
              ...
              <button
                onClick={sendMessage}
                disabled={!input}
                className="inline-flex h-12 w-12 items-center justify-center rounded-full bg-slate-900 text-white shadow-lg shadow-slate-900/20 transition hover:bg-slate-800 disabled:cursor-not-allowed disabled:opacity-50"
              >
                ➤
              </button>
            </div>
